...
export default function MenuPlannerClient({ providerId, initialMenus, initialHistoryMenus, initialQuickTags, initialWeekStart, initialToday }: Props) {
  ...
  return (
    <div onClick={() => setActiveHelp(null)} className="min-h-screen bg-[#FDF8F3] pb-[calc(7rem+env(safe-area-inset-bottom))]">
      <header className="fixed inset-x-0 top-0 z-40 bg-white/95 backdrop-blur-xl border-b border-orange-100/70 px-4 py-3 shadow-[0_4px_30px_rgba(244,98,42,0.05)]">
      ...
      <main className="mx-auto max-w-2xl px-4 pt-24 space-y-4">
        ...
        {!!smartSuggestions.length && (
          <section className="rounded-[1.5rem] border border-orange-100 bg-orange-50/60 p-4">
            <h2 className="mb-2 flex items-center gap-2 text-sm font-black text-orange-700">
              <Lightbulb className="w-4 h-4" />
              Smart nudges
            </h2>
            <div className="space-y-1.5">
              {smartSuggestions.map(suggestion => (
                <p key={suggestion} className="text-xs font-bold leading-relaxed text-orange-700/80">{suggestion}</p>
              ))}
            </div>
          </section>
        )}
