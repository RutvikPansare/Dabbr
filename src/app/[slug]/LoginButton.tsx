...
import { LogIn, X, AlertCircle, Phone, RefreshCw, Loader2 } from 'lucide-react'
import { useRouter } from 'next/navigation'

type Step = 'default' | 'phone' | 'otp' | 'finding'
...
export default function SlugLoginButton({ providerSlug }: { providerSlug: string }) {
  ...
  async function handleGoogle() {
    ...
    if (isNative) {
      try {
        ...
        // On native: signed in, now find portal token
        setStep('finding')
        await findAndRedirect()
      } catch (err: any) {
      ...
      // Web: OAuth redirect → callback → /customer-redirect/[slug]
      await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: `${window.location.origin}/auth/callback?next=/customer-redirect/${providerSlug}`,
        },
      })
      // Page redirects; loading stays
    }
  }
  ...
  async function findAndRedirect() {
    try {
      ...
      setError(e.message ?? 'Could not find your subscription.')
      setStep('default')
      setLoading(false)
    }
  }
            ...
            <div className="px-5 py-5 space-y-3">
              {step === 'finding' ? (
                <div className="flex flex-col items-center gap-3 py-8">
                  ...
                  <p className="text-sm font-semibold text-gray-500">Finding your subscription…</p>
                </div>
              ) : step === 'otp' ? (
                /* ── OTP entry ── */
