...
export default function LoginForm({ refCode }: { refCode?: string }) {
  ...
  async function handleGoogle() {
    ...
    if (isNative) {
      try {
        ...
        if (!userCancelled) setGoogleError(msg || 'Google sign-in failed.')
        setGoogleLoading(false)
      }
    } else {
  ...
  // Default + phone-expanded step (Google primary, phone secondary)
  return (
    <div className="space-y-3">
      ...
      {googleError && (
        <p className="text-center text-xs font-semibold text-red-500">{googleError}</p>
      )}
      ...
      {/* ── Phone — secondary, expands on tap ── */}
      {step === 'default' ? (
        <button
          type="button"
