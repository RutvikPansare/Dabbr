...
export default function GoogleSignInButton() {
  ...
  async function handleSignIn() {
    ...
    if (isNative) {
      // ── Native Android/iOS: account picker sheet, no browser ─────────────
      try {
        ...
        // Exchange Google idToken with Supabase
        ...
        if (err?.message?.includes('cancel') || err?.message?.includes('12501')) {
          // User dismissed — just stop loading
        } else {
          setError('Google sign-in failed. Please try Phone OTP instead.')
        }
        setLoading(false)
      }
    } else {
