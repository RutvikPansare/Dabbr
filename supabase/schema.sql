...
-- ============================================================
-- MENU QUICK TAGS
-- Provider-owned reusable menu chips for faster planning.
-- ============================================================
create table public.menu_quick_tags (
  id            uuid primary key default uuid_generate_v4(),
  provider_id   uuid not null references public.providers(id) on delete cascade,
  meal_slot     text not null check (meal_slot in ('breakfast', 'lunch', 'dinner')),
  plan_type     text check (plan_type in ('veg', 'nonveg')),
...
create index menu_quick_tags_provider_slot_type_idx
  on public.menu_quick_tags (provider_id, meal_slot, coalesce(plan_type, 'any'), sort_order);

create unique index menu_quick_tags_provider_slot_type_label_key
